...
try {
  // List connections using tenant settings
  const { stdout: existingConnections } = await $`auth0 tenants connections list --json`
  const connections = JSON.parse(existingConnections)
  const existingConnection = connections.find(conn => conn.name === DEFAULT_CONNECTION_NAME)
  
  if (existingConnection) {
    defaultConnection = existingConnection
    createDatabaseConnection.succeed(`Using existing ${DEFAULT_CONNECTION_NAME} connection`)
  } else {
    // prettier-ignore
    const createConnectionArgs = [
      "tenants", "connections", "create",
      "--strategy", "auth0",
      "--name", DEFAULT_CONNECTION_NAME,
      "--display-name", "SaaStart",
      "--enabled-clients", `${dashboardClient.client_id},${managementClient.client_id}`,
      "--json"
    ]

    const { stdout } = await $`auth0 ${createConnectionArgs}`
    defaultConnection = JSON.parse(stdout)
    createDatabaseConnection.succeed(`Created new ${DEFAULT_CONNECTION_NAME} connection`)
  }
} catch (e) {
...
const createAdminRole = ora({
  text: `Getting or creating admin role`,
}).start()
let adminRole
try {
  // First try to get existing role
  const { stdout: existingRoles } = await $`auth0 roles list --json`
  const roles = JSON.parse(existingRoles)
  const existingAdminRole = roles.find(role => role.name === "admin")
  
  if (existingAdminRole) {
    adminRole = existingAdminRole
    createAdminRole.succeed(`Using existing admin role`)
  } else {
    // prettier-ignore
    const createRoleArgs = [
      "roles", "create",
      "--name", "admin",
      "--description", "Manage the organization configuration",
      "--json", "--no-input"
    ];

    const { stdout } = await $`auth0 ${createRoleArgs}`
    adminRole = JSON.parse(stdout)
    createAdminRole.succeed(`Created new admin role`)
  }
} catch (e) {
  createAdminRole.fail(`Failed to get or create the admin role`)
  console.log(e)
  process.exit(1)
}

const createMemberRole = ora({
  text: `Getting or creating member role`,
}).start()
let memberRole
try {
  // First try to get existing role
  const { stdout: existingRoles } = await $`auth0 roles list --json`
  const roles = JSON.parse(existingRoles)
  const existingMemberRole = roles.find(role => role.name === "member")
  
  if (existingMemberRole) {
    memberRole = existingMemberRole
    createMemberRole.succeed(`Using existing member role`)
  } else {
    // prettier-ignore
    const createRoleArgs = [
      "roles", "create",
    ...
    createMemberRole.succeed(`Created new member role`)
  }
} catch (e) {
  createMemberRole.fail(`Failed to get or create the member role`)
  console.log(e)
  process.exit(1)
}
...
const createSecurityPoliesAction = ora({
  text: `Getting or creating Security Policies Action`,
}).start()
let securityPoliciesAction
try {
  // First try to get existing action
  const { stdout: existingActions } = await $`auth0 actions list --json`
  const actions = JSON.parse(existingActions)
  const existingAction = actions.find(action => 
    action.name === "Security Policies" && 
    action.supported_triggers[0]?.id === "post-login"
  )
  
  if (existingAction) {
    securityPoliciesAction = existingAction
    createSecurityPoliesAction.succeed(`Using existing Security Policies Action`)
  } else {
    ...
    createSecurityPoliesAction.succeed(`Created new Security Policies Action`)
  }
} catch (e) {
  createSecurityPoliesAction.fail(
    `Failed to get or create the Security Policies Action`
  )
  console.log(e)
  process.exit(1)
}
...
const createAddDefaultRoleAction = ora({
  text: `Getting or creating Add Default Role Action`,
}).start()
...
try {
  ...
  const actions = JSON.parse(existingActions)
  const existingAction = actions.find(action => 
    action.name === "Add Default Role" && 
    action.supported_triggers[0]?.id === "post-login"
  )
  
  if (existingAction) {
    addDefaultRoleAction = existingAction
    createAddDefaultRoleAction.succeed(`Using existing Add Default Role Action`)
  } else {
    ...
    await $`auth0 ${deployActionArgs}`
    createAddDefaultRoleAction.succeed(`Created new Add Default Role Action`)
  }
} catch (e) {
...
const createAddRoleToTokensAction = ora({
  text: `Getting or creating Add Role to Tokens Action`,
}).start()
...
try {
  ...
  const actions = JSON.parse(existingActions)
  const existingAction = actions.find(action => 
    action.name === "Add Role to Tokens" && 
    action.supported_triggers[0]?.id === "post-login"
  )
  
  if (existingAction) {
    addRoleToTokensAction = existingAction
    createAddRoleToTokensAction.succeed(`Using existing Add Role to Tokens Action`)
  } else {
  ...
  await waitUntilActionIsBuilt(addRoleToTokensAction.id)
    ...
    createAddRoleToTokensAction.succeed(`Created new Add Role to Tokens Action`)
  }
} catch (e) {
  createAddRoleToTokensAction.fail(
    `Failed to get or create the Add Role to Tokens Action`
  )
  console.log(e)
  process.exit(1)
}
